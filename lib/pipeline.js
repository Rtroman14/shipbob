...
import { fetchMerchantHistory } from "./db.js";
...
export const gatherData = async (case_id, api) => {
    ...
    const [attachmentsRes, shipment, order, merchantHistory] = await Promise.all([
        api(`/cases/${case_id}/attachments`),
        api(`/shipments/${caseData.shipment_id}`),
        api(`/orders/${caseData.order_id}`),
        fetchMerchantHistory(caseData.account_name),
    ]);

    const attachments = attachmentsRes.attachments || [];
    const rawInvoice = generateInvoice(caseData.shipment_id, caseData.user_id);
    const invoice = rawInvoice.error ? { line_items: [] } : rawInvoice;
    ...
    return { caseData, caseSummary, attachments, shipment, order, invoice, merchantHistory };
};
...
export const analyzeImages = async (attachments, caseDescription, invoiceLineItems) => {
    const system = generatePrompt("prompts/image-analysis-system.md");
        ...
        attachments.map(async (attachment) => {
            const { output } = await generateText({
                ...
                output: Output.object({
                    schema: z.object({
                        classification: z
                            .array(z.enum(["product_damage", "outer_packaging", "other", "unclear"]))
                            .min(1),
...
export const makeDecision = (imageAnalyses, invoice) => {
    const hasProductDamage = imageAnalyses.some(
        (img) => img.classification.includes("product_damage") && img.damage_visible
    );
    const hasOuterPackaging = imageAnalyses.some((img) =>
        img.classification.includes("outer_packaging")
    );
    ...
    if (verifiedMatches.length === 0) {
        const hasIdentifiableProduct = imageAnalyses.some((img) => img.product_identifiable);
        if (hasIdentifiableProduct) {
            reasons.push("Product visible in photos but does not match any invoice line item");
        } else {
            reasons.push("Cannot identify any product in the submitted photos");
        }
    }
    ...
    const reimbursementItems = uniqueMatched.map((name) => {
        ...
        const lineTotal = invoiceItem.unit_price * (invoiceItem.quantity || 1);
        return { product_name: name, quantity: invoiceItem.quantity || 1, invoice_price: invoiceItem.unit_price, amount: lineTotal };
    });
